...
| 2026-09-08 | Class features | Data-driven from `CLASS_FEATURES` (Rogue + Barbarian, levels 1–20, mechanics paraphrased from SRD 5.2 / CC-BY-4.0). `derivedClassFeatures(sheet)` — no storage, no manual entry; rendered as a read-only section. Semi-automatic: Rogue Sneak Attack is an "armed" one-shot (`sneakAttackArmed`) that `actionDamageParts` folds in and the sheet disarms; Barbarian Rage is a manual toggle (`raging`) + charge tracker (`rageUsed`, max derived) that auto-adds rage damage to weapon attacks and grants b/p/s resistance via `derivedDefenses`. Rage never auto-ends — deliberate (RP rages). Subclasses out of scope. |
