...
- **Cantrips** (0.26.0). `shared/src/cantrips.ts` — `CantripDef` list paraphrasing
  SRD 5.2 (CC-BY-4.0) mechanics, each with a Vietnamese `guidance` line. The
  Spell tab's "+ Cantrip SRD" picker groups them by the sheet's class list vs.
  the rest and by combat / utility. `spellFromCantrip(def, sheet, id)` builds a
  ready spell: cast kind, damage scaled by `cantripTier` (1/2/3/4 dice at char
  level 1/5/11/17 — `scaleCantripDie`), and the save ability for save cantrips.
  `rescaleCantripSpell` re-scales an existing cantrip row (⟳ button). Save
  cantrips carry their damage in `Spell.damage`; the cast flow passes it as
...
- **Level-1..3 combat spells** (0.28.0 / 0.29.0 / 0.35.0). The SRD spell DB
  ...
  SRD" / "+ cấp 2 SRD" pickers mirror the cantrip picker. `addSpellMod` bakes
  the caster's spell modifier into a leveled attack's damage (Spiritual Weapon).
  Concentration spells whose branch doesn't place a target effect (Witch Bolt,
  Moonbeam, Cloud of Daggers, …) drop a "Đang tập trung: X" marker on the
  caster's own token so the one-concentration rule still fires. Buffs
  (Bless/Bane/Shield/Mage Armor) and rare utility stay guidance-only; anything
  not in the DB is added by hand with "+ Thêm phép".
...
- **Weapon / spell attack tag** (0.27.0, reworked 0.28.0). `DamageRider.scope` =
  ...
  concentration effect also ends the caster's concentration, so a used Guidance
  ends the spell — 5e-correct.
...
| 2026-09-10 | Spell range vs. AoE (0.36.0) | `Spell.area` / `CantripDef.area` = the AoE template string, separate from `range` (pure casting range). ~40 defs fixed: Self line/cone/emanation spells → `range: 'Bản thân'` + template in `area`; Beacon of Hope → 30 ft. Cast banner + picker show both; the map range ring uses `max(parseRangeFeet(range), parseRangeFeet(area))`. |
